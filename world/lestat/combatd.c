...
void killer_reward(object killer, object victim)
{
    ...
    if(killer->query_temp("pkmeet/group") == victim->query_temp("pkmeet/group")
         && killer->query_temp("pkmeet/baoming") && victim->query_temp("pkmeet/baoming")
         && killer->query_temp("pkmeet/begin") && victim->query_temp("pkmeet/begin") )
        {
         killer->add_temp("pkmeet/score",victim->query_temp("pkmeet/score"));
         if(dan=present("huanhun dan", victim))
             {if ( !present("huanhun dan", killer) )             
                  {dan->move(killer);
                   tell_object(killer,HIG"你得到了九转还魂丹。\n"NOR);
                  }
               else destruct(dan);
             }
         if ( !present("huanhun dan", killer) )
               {new("/d/obj/drug/dan")->move(killer);
                tell_object(killer,HIG"你得到了九转还魂丹。\n"NOR);
               }
         return;
        }
