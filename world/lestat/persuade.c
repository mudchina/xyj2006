...
int main(object me, string arg)
{
    ...
    if ( (!(myfam = me->query("family")) || myfam["family_name"] != "南海普陀山") )
        return notify_fail("只有佛门中人才能用渡世济人！\n");
    ...
    if ( (vtfam = victim->query("family")) && vtfam["family_name"] == "佛界" )
...
void compelete_dushi(object me, object victim, object ob, int sp, int dp)
{
	...
	if( living(victim) && (random(sp+dp) > dp) ) 
	{
		...
		me->improve_skill("buddhism", random(me->query("int")));
