...
"xyj":  ([  "msg_speak": HIR "【西游记2006】%s：%s\n" NOR,
    ...
    "chat": ([  "msg_speak": HIW "【"HIG"太平盛世"HIW"】"HIC"%s：%s\n" NOR,
                "msg_emote": HIW "【"HIG"太平盛世"HIW"】"HIC"%s\n" NOR,
			 ]),
    "rumor":([  "msg_speak": HIM "【江湖传闻】%s：%s\n" NOR,
                "msg_emote": HIM "【江湖传闻】%s\n" NOR,
				"anonymous": "某人"
			]),
	...
	int allow_chat)
{
	...
	if( userp(me) && !allow_chat ) {
		if(channels[verb]["wiz_only"] && !wizardp(me) )
			return 0;
        if(channels[verb]["party_only"] && !me->query("bang/name"))
		return notify_fail("你还是先加入一个帮派再说吧。\n");

	if (channels[verb]["party_only"]) {
		party = me->query("bang/name");
...
//as followed add by vikee,only belone to the party can see the message
	if (ch["party_only"])
		return (wizardp(ppl)
		&& (!ppl->query("env/party") || ppl->query("env/party")==party))
		|| party == ppl->query("bang/name");
//changed by beeby          ppl->query("family/family_name"); 
	return 1;
}
