...
void create()
{
          set_name(HIG"XLQY常德站新闻版"NOR, ({ "news"}) );
...
int check_news(object me)
{
	...
	//				CHINESE_D->chinese_time(5, ctime(notes[i]["time"])));
		...
		if(i>10) write("你可以 Help news 来查看XLQY常德站新闻系统的使用方法。\n"); //i>10 提醒不知道有NEWS系统的玩家使用news
	}
}
...
int main(object me, string arg)
{
	...
	if( !pointerp(notes) || !sizeof(notes) )
		return notify_fail(HIC"「XLQY常德站」目前没有任何新闻发布。\n"NOR);
		
	if( !arg ) 
	{
		if(i>10) write(HIC"你可以"HIR" Help news "HIC"来查看XLQY常德站新闻系统的使用方法。\n"NOR); //10
			return notify_fail(
"「XLQY常德站」目前共有"+HIW+chinese_number(sizeof(notes))+NOR"条新闻"+((i==0)? "。":"，您有"+HIY+chinese_number(i)+NOR"条新闻尚未阅读。")+"\n"); }
...
//	me->start_more( sprintf("\n\n%s\n"HIR"新闻主题："HIY"%s\n"HIW"新闻巫师："NOR"%s\n"HIG"新闻时间：%s\n"NOR
	...
	CYN"-------------------------------------------------------------------------------\n"NOR,
		BBLU HIG"「"HIR"XLQY常德站"HIG"」第"+chinese_number(num + 1)+"条新闻 "NOR, 
...
void done_post(object me, mapping note, string text)
{
	...
	restore();
// 使用channel显示提示有了最新新闻
message("channel:rumor", HIR"【新闻发布】："HIG"「XLQY常德站」"HIY"有了最新更新新闻，请用 "HIR"news"NOR HIY" 查看！！！ \n"NOR, users());
                          //By waiwai@mszj 2000/11/13 
