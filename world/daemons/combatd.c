	...
	if( random(ap + dp) < mod_val ) { 		// Does the victim dodge this hit?
		...
		if( random(ap + pp)< mod_val ) {
			...
			if( weapon ) {
				...
				if( sizeof(victim->query_temp("armor")) )
	{
          for(mod_val=0; mod_val<sizeof(victim->query_temp("armor")); mod_val++)
          {
          foo =	values(victim->query_temp("armor"))[mod_val]->ob_hit_msg(victim,
me,damage_bonus);
          if( stringp(foo) ) result += foo;
          foo =	values(victim->query_temp("armor"))[mod_val]->ob_hit_int(victim,
me,damage_bonus);
