...
string _query_rank(object ob)
{
	...
	{
	case "女性":
               switch(SECURITY_D->get_status(ob)) {
			case "(admin)":	        return HIG "【 女  神 】" NOR;
			case "(arch)":	        return HIC "【 巫  女 】" NOR;
			case "(wizard)":	return HIG "【 天   女】" NOR;
			case "(apprentice)":	return HIC "【纪律主管】" NOR;
                     case "(immortal)":         return HIC "【见习天女】" NOR;
                     case "(elder)":            return HIR "【宣传部长】" NOR;
			default:
...
//begin 男性：
	default:
              switch(SECURITY_D->get_status(ob)) 
		{
			...
			case "(arch)":	        return HIW "【 大  神 】" NOR;
			case "(wizard)":        return HIW "【 神  仙 】" NOR;
			case "(apprentice)": 	return HIC "【 散  仙 】" NOR;
                        case "(immortal)":      return HIC "【 半  仙 】" NOR;
			case "(elder)": 	return HIR "【荣誉玩家】" NOR;
			default:
				switch(ob->query("class")) 
				{
//------------------rank for men--------------------
