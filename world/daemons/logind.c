...

{
    ...
    write("         [[1;31mÌáÊ¾£º[0;37;0mÈç¹û°´"HIY"ENTER"NOR"¼üºóÏµÍ³Ã»ÓĞÏìÓ¦£¬ÇëÊÔ Ctrl-ENTER]\n\n");

    write("                  »ÃÏëÎ÷Ìì»¶Ó­ÄãµÄµ½À´!Select GB or BIG5(gb/big5):");
...
// try strict ban here...weiqi.
	...
	// vikee 2001-2-15 17:42
          ...
          printf("»ÃÏëÎ÷Ìì["HIG"»ªÖĞÕ¾"NOR"]ÏÖÓĞ×¢²áÍæ¼Ò"HIR"%s"NOR"Î»¡£\n",chinese_number(howmany_user()));
//	printf("Î÷ĞĞÕ½¼Ç["HIY"±±¾©Õ¾"NOR"]¿ªÕ¾ÒÔÀ´£¬×î¸ßÔÚÏßÊıÄ¿£º"HIY"%s"NOR" ÈË£¬Æ½¾ùÔÚÏßÊıÄ¿£º"HIY"%s"NOR" ÈË¡£\n",chinese_number(max_online),chinese_number(avg_online));

	printf("Ä¿Ç°¹²ÓĞ"HIR"%s"NOR"Î»¹ÜÀíÈËÔ±¡¢"HIG"%s"NOR"Î»Íæ¼Ò£¬ÒÔ¼°"HIY"%s"NOR"Î»Ê¹ÓÃÕßÔÚ³¢ÊÔÁ¬Ïß¡£\n\n",
...
*/
	...
	write("ÔÚÄú½øÈëÓÎÏ·Ç°,»ÃÏëÎ÷ÌìÌáĞÑÄú²»ÒªÍü¼ÇÄúµÄ¹¤×÷»òÕßÑ§Ï°\n");
...
// mon 7/19/97 to prevent flooding by repeating illegal id.
	...
	write("\nÄúµÄÓ¢ÎÄÃû×Ö£º£¨ĞÂÍæ¼ÒÇë¼üÈë"HIW"New"NOR"×¢²á£©");
...
ÇëÄú¸ø×Ô¼ºÏëÒ»¸ö·ûºÏ¡¼»ÃÏëÎ÷Ìì¡½Éñ»°ÊÀ½çµÄÖĞÎÄÃû×Ö¡£Õâ

¸öÃû×Ö½«´ú±íÄãÔÚ»ÃÏëÎ÷ÌìÖĞµÄÈËÎï£¬¶øÇÒÍùºó½«²»ÄÜÔÙ¸ü¸Ä¡£
...

{
	...
	user->set("no_gift",1);
	    ...
	    user->set("channels", ({ "chat","rumor","hxxt","sldh","es","job" }) );

	    log_file( "USAGE", sprintf("%s was created from %s (%s)\n", user->query("id"),

		query_ip_number(ob), ctime(time()) ) );
    CHANNEL_D->do_channel( this_object(), "rumor","ÌıËµÒ»¿ÅÁ÷ĞÇ×¹ÂäÔÚ³¤°²¸½½ü£¬"+user->query("name")+"("+user->query("id")+")À´µ½ÁËÈËÊÀ£¬´Ó´Ë¿ªÊ¼ÁËËû(Ëı)µÄÎ÷ĞĞÖ®Â·¡£"NOR);
...
{            
	...
	"/adm/daemons/toptend"->topten_checkplayer(user);
		...
		tell_room(startroom, user->query("name") + "Á¬Ïß½øÈëÕâ¸öÊÀ½ç¡£\n",
			...
			({user}));
//    message("channel:chat", HIM"¡¼ÔÙÕ½½­ºş¡½: "HIC"¾İËµ"+HIW+user->name()+HIC "À´µ½ÁË»ÃÏëÎ÷Ìì£¬¼ÌĞø×ÅËûÎ´Íê³ÉµÄÂ·Í¾\n"NOR,users());
   		...
   		if(file_size("/log/notice")>0){
			...
			write(CYN"     ¡Ô¡Ô¡Ô¡Ô¡Ô¡Ô¡Ô¡Ô¡Ô¡Ô¡Ô¡Ô"HIR"»ÃÏëÎ÷ÌìÁ¬ÏßÍ¨Öª"NOR CYN"¡Ô¡Ô¡Ô¡Ô¡Ô¡Ô¡Ô¡Ô¡Ô¡Ô¡Ô¡Ô\n");
