...
//      add by dragon for PK rumor
	...
	{	
	  ...
	  message("channel:xyj",HIR+"【大唐通告】薛仁贵(xueren gui)："+killer->name(1)+"无视王法，滥伤人命，打死了"+victim->name(1)+"，现正被官府通缉！\n"+NOR,users());
	  ...
	  if(killer->query("tongji/pktime")>=3)
          ...
          message("channel:xyj",HIR+"【大唐通告】薛仁贵(xueren gui)："+killer->query("name")+"连续杀伤人命，现被关进大牢，待秋后发落！\n"NOR,users());
            } 
	}
	else if(victim->query("tongji/fanren"))
	{
	
	message("channel:xyj",HIR+"【大唐通告】薛仁贵(xueren gui)："+killer->name(1)+"杀死通缉犯"+victim->name(1)+"有功，现通报嘉奖！\n"+NOR,users());
	...
	message("channel:xyj",HIR+"【大唐通告】薛仁贵(xueren gui)："+killer->name(1)+"失手打死了"+victim->name(1)+"，现已逃命，望知其下落者，速报知官府！\n"+NOR,users());
	killer->set("tongji/fanren",1);
	killer->set("tongji/mudage",killer->query("mud_age"));
	killer->add("tongji/pktime",1);
	//added for prison
...
if(killer->query("tongji/pktime")>=3)
 {
 killer->move("/d/wiz/dalao");
  message("channel:xyj",HIR+"【大唐通告】薛仁贵(xueren gui)："+killer
