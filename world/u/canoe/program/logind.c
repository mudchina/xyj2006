...
printf("西游记2006("HIY"总站"NOR")现有注册玩家"HIR"%s"NOR"位。\n",chinese_number(howmany_user()));
...
// wiz上线通知
if (!wizardp(user))
   CHANNEL_D->do_channel( this_object(), "sys",sprintf(NOR WHT "%s由 " HIG "%s" NOR WHT "连线进入,IP来自："HIG"%s"NOR,user->name()+"("+capitalize(user->query("id")) + ")", query_ip_number(user), ip_from)); 
else
    CHANNEL_D->do_channel( this_object(), "wiz",sprintf("%s由 %s连线进入,IP来自：%s",user->name()+"("+capitalize(user->query("id")) + ")", query_ip_number(user), ip_from)); 
// over 上线通知
		}
	...
	qqfriends = keys(user->query("qq"));
	 for(qqnum=0;qqnum<sizeof(qqfriends);qqnum++)
				...
				tell_object(qqob,BLINK HIG+"你的好友"+user->query("name")+"["+capitalize(user->query("id"))+"]上线了！！\n"+NOR);
		}
         }
     ////////////////////////////////////////////////////////
