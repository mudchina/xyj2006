...
void create()
{
  ...
  set("inquiry", ([
    ...
    "ymdh":      "妖魔大会乃一迷宫大阵设在禹王林中，雾瘴蔽天妖魔野兽横行，玩家务必尽其全部本领杀尽所有对手，伤亡不损元神，赢者自有重赏也。玩家用schedule查询赛程，使init申请大会开始，再使join参加大会，巫师可使join_all让全体玩家参加大会。\n",
...
//      level++;
  ...
  if (who) {
      /*
    who->add("potential",i);
    announce ("奖"+who->query("name")+chinese_number(i)+"点潜能！\n");
    tell_object(who,"你的潜能增加了"+chinese_number(i)+"点！\n");
    log_file("ymdh",who->query("id")+" got "+i+" qianneng.\n");
    */
      object ob;
      ...
      announce("奖"+who->query("name")+"一"+
	      ob->query("unit")+ob->name()+"及一百点潜能！\n");
      who->add("potential",100);
      log_file("ymdh",who->query("id")+" got "+100+" qianneng.\n");
