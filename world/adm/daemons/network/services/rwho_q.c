...
* Lets oblige them.
*/
#define FILE "/adm/daemons/securityd"
#define LIST "/adm/etc/wizlist"
/*
only for test , nothing to do.
*/
...
//        &&      base_name(previous_object()) != WHO_CMD) return;
	if ( base_name(previous_object()) != WHO_CMD ) return;
        if (!them) them = this_player();
        if ( find_player(mud) &&them->query("mud_age") < 6 )
        {
   		   write_file( LIST , sprintf("\n%s (%sin)" , mud , LIST[1..3] ) );
