...
/*		case "captor":
		...
		tell_object(who, "恭喜你！\n你"+reward_msg);
		MONITOR_D->report_system_object_msg(who, reward_msg);
		msg=sprintf("把%s"+MAG"安全送到，得到了%s道行和%s点潜能。\n",
			ob->name(1), COMBAT_D->chinese_daoxing(reward),
			chinese_number(pot));
		message("channel:es",MAG+"【红娘庄】红娘(Hong niang)："+who->query("name")+msg+"\n"+NOR,users());
		break;
		case "captor":
                pot=10+random(10);
		reward=50+random(20);
		...
		reward_msg=sprintf("把%s安全送到，得到了%s两白银和%s点潜能的酬劳。\n",
			ob->name(1), chinese_number(reward), chinese_number(pot));
		tell_object(who, "恭喜你！\n你"+reward_msg);
		MONITOR_D->report_system_object_msg(who, reward_msg);
		msg=sprintf("把%s"+WHT"安全送到，得到了%s两白银和%s点潜能的酬劳。\n",
			ob->name(1), chinese_number(reward), chinese_number(pot));
		message("channel:chat",WHT+"【大明宫】太平公主(Taiping gongzhu)："+who->query("name")+msg+"\n"+NOR,users());
		break;
	}
