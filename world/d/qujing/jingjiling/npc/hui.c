...
void create()
{
        set_name("Áè¿Õ×Ó", ({"lingkong zi", "zi"}));
        set("age", 90);
...
int accept_fight(object ob)
{
	object me = this_object();
        ...
        if(me->is_fighting() )
                return notify_fail("Áè¿Õ×Ó¶ÔÄãËµ£ºÃ»¼ûÀÏ·òÕýÃ¦×ÅÄÇÃ´£¿\n");
        if( (int)me->query("kee")*100/(int)me->query("max_kee") < 30 )
                return notify_fail("Áè¿Õ×Ó¶ÔÄãËµ£ºÀÏ·òÌå·¦ÎÞÁ¦£¬±È²»µÃ£¬±È²»µÃ¡£\n");
...
{
        object me=this_object();
        message_vision("$N¶Ô$nÎÊµÀ£º¡°Ã÷ÔÂ³öÌìÉ½¡±µÄÏÂ¾äÊÇÊ²Ã´£¿\n", me, who);
