...
void create()
{
        set_name("·÷ÔÆÛÅ", ({"fuyun sou", "sou"}));
        set("age", 90);
	set("attitude", "peaceful");
        set("gender", "ÄÐÐÔ");
	set("class", "yaomo");
	set("title", "ÉîÉ½ËÄ²Ù");
	set("nickname", "¸êÓñÇÃ½ð");
        set("str", 20);
        set("int", 20);
        set("per", 30);
        set("con", 20);
...
int accept_fight(object ob)
{
	...
	if(me->is_fighting() )
		return notify_fail("·÷ÔÆÛÅ¶ÔÄãËµ£ºÃ»¼ûÀÏ·òÕýÃ¦×ÅÄÇÃ´£¿\n");
	if( (int)me->query("kee")*100/(int)me->query("max_kee") < 30 )
		return notify_fail("·÷ÔÆÛÅ¶ÔÄãËµ£ºÀÏ·òÌå·¦ÎÞÁ¦£¬±È²»µÃ£¬±È²»µÃ¡£\n");
...
{	
	object me=this_object();
	message_vision("$N¶Ô$nÎÊµÀ£º¡°ÎåÔÀÑ°ÏÉ²»´ÇÔ¶¡±µÄÏÂ¾äÊÇÊ²Ã´£¿\n", me, who);
...
int do_answer(string arg)
{
	...
	if( this_player()->query_temp("answer_zhu") ) {
		...
		if( strsrch(arg, "Ò»ÉúºÃÈëÃûÉ½ÓÎ") >=0  ) {
                command("haha");
