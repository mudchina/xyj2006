...
void create()
{
  set_name(HIR"丹凤"+HIW"锏"NOR, ({ HIR"gufeng mace"NOR, "mace", "mace" }) );
        set_weight(4000);
        if( clonep() )
            ...
            set("long", "这把叉是由天神帝释的舍利子所转化的。\n");
                set("wield_msg", "一道暗红闪过，$N手中已多了把$n。\n");
        }
 set("no_sell", 1);
 set("no_drop", 1);
 set("no_put",1);
...
int ji_ob(object victim)
{
        ...
        fabao->set("spi", 0);
         fabao->add("spi", 6000);
        if( me->is_busy() )
return notify_fail("你正忙着呢，无法祭出"+HIR"丹凤"+HIW"锏。\n"NOR);
        if( !victim )
                return notify_fail("你想祭的人不在这里。\n");
        if( where->query("no_magic") )
     return notify_fail("这里不能祭"+HIR"丹凤"+HIW"锏。\n"NOR);
        if( !me->is_fighting() )
                return notify_fail("只有战斗中才能祭出。\n");
        if( (int)me->query("mana") < 800 )
  return notify_fail("你的法力不能控制"+HIR"丹凤"+HIW"锏。\n"NOR);

message_vision(YEL"\n$N大喝一声“去!”，只见$n"+YEL"「呼」地一下化做一条巨龙飞到半空。\n"NOR, me, fabao);

message_vision(YEL"霎那间天色一变！$n"+YEL"呼啸着向"+HIW"$N"+YEL"迎面打下！\n"NOR, victim, fabao);
...
message_vision(WHT"\n$N手一指大喝一声“来！”，只听$n"+WHT"发出一声轻哮！飞到了"+HIR"$N"+HIW"手中。\n"NOR, victim, fabao);
                ...
                else
                {
                     ...
                     if( damage > 3000 ) damage = 3000; 
                        ...
                        if( damage > 0 )
                        {
message_vision(HIY"\n结果$N"+HIY"来不及闪开，白光闪过之后，$n"+HIY"已经在"+HIW"$N"+HIY"的身体上打了几锏,\n"+HIY"        过了一会，鲜血才喷出来！\n\n"+"        $n"+HIW"通过这次实践,攻击力加强了.\n"NOR, victim, fabao,victim);
