...
int do_abort(string arg)
{
	...
	for(i=0; i<NUM; i++) {
	  ...
	  for(i=0; i<NUM; i++) {
	    ...
	    if(!interactive(obj) || query_idle(obj) > 240 ) 
		leave=1;
	    else 
		continue; // only disconnected or idle player leave.
...
//	return notify_fail("你的积分过低不能参加比赛。\n");
    ...
    rank1=pig->query_id_rank(me->query("id")); // re-check rank.
//    if(rank1>=100 && rank1<rank) {
// won't lose point for no-show    xlb 2.4.99
    if(rank1 <100 && rank1>=100 && rank1<rank) {
	me->set("piggy/rank",rank1);
        message_vision("拱猪王走了过来，在$N耳边悄声说道："+
            "因为你最近没来拱猪，等级分减少了"+
             chinese_number(rank-rank1)+"点：（\n", me);
        log_file("pig_log",ctime(time())+
		me->query("id")+" lost "+(rank-rank1)+" points.\n");
    }
...
void update_one_rank(object obj, int k)
{
    object tmp, pig;
    int new_rank;
// added on 2/99, 10(N) hands played rewarded 1 rank point.
  int N=10, seniority, new_seniority;
	...
	if(k>0) {
	    if(random(40)==0 && k<4) {
		message_vision("太幸运了！$N得到了猪头大奖，"+
			"奖励分加倍！"+
			"$N的等级分增加了"+chinese_number(k+k)+
			"点。\n",obj);
		new_rank+=k;
	    } else {
	      message_vision(
...
//xlb: added on 2/99, 10(N) hands played rewarded 1 rank point.
   seniority=obj->query("piggy/seniority");
  new_seniority=obj->query("piggy/hand_played")/N;
   new_seniority=new_seniority-seniority;
  if (new_seniority>seniority) {
     obj->set("piggy/seniority",new_seniority);
     new_rank=new_rank+new_seniority-seniority;
   }
