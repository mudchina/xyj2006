...
me = this_player();
                ...
                command( "consider");
                command( "say Ì«ÒÒÕæÈËµÄ½ğ×©ËÆºõÔÚ.....");
                command( "say ÎÒ¸øÄãÒ»´®ÑÌÅÚ£¬ÎÒÃÇÒ»ÆğÈ¥ÃØÃÜµÄµØ·½È¼·ÅºÃÃ´£¿");
        bomb = new("/d/sanxian/bomb");
        if(bomb)
        bomb->move(me);
                command( "say ÎÒÔÙ¸øÄãÒ»ÕÅÖ½£¬ÄãÒª(remember)Å¶¡£µ½Ö½ÉÏÌáÊ¾µÄµØ·½È¥·ÅÑÌÅÚ£¬¿ÉÄÜÓĞºÃ¶«Î÷Ò®¡£");

        paper = new("/d/sanxian/paper");
        if(paper)
        paper->move(me);
        return 1;
}
