...
void init()
{
	...
	object who = this_player();
        ...
        me->create_identity("/d/kunlun/npc/jiang", "/d/kunlun/dadian");
...
void reset_me (object me)
{
	...
	//me->set("title",me->query("family/family_name")+zname(me));
        me->set("title", zname(me)); //for kunlun only, others use above.
...
int convert_identity (object me, object ob)
{
	...
	//ob->set("zhangmen/title",ob->query("family/family_name")+zname(ob));
        ob->set("zhangmen/title", zname(ob)); //for kunlun only, others use above.
	//ob->set("title",ob->query("family/family_name")+zname(ob));
        ob->set("title", zname(ob)); //for kunlun only, others use above.
	ob->save();
	...
	//me->set("title",ob->query("family/family_name")+zname(ob));
        me->set("title", zname(ob)); //for kunlun only, others use above.
	me->set("short",me->query("name")+"("+capitalize(me->query("id"))+")");
...
string zm_apply()
{
	...
	if (me->query("current_player") == ob->query("id")) 
	{
        ...
        ob->set_temp("kunlun_zm_applied", 1);
        return "我昆仑山玉虚洞乃玄门正宗，欲做掌门，没本事不行，就请一战吧。\n";
}
...
int accept_fight(object ob)
{
	...
	if (me->is_fighting())
	{
        ...
        if (ob->query_temp("kunlun_zm_applied"))
	{
...
int check_result(object me, object ob)
{
	...
	if (((int)me->query("kee")*100/(1+my_max_kee)) <= 50 )
	{
		...
		message_vision ("$N翻身下拜，连声佩服！\n",me);
                ...
                if (me->query_temp("zhangmen/kill") && ob->query_temp("kunlun_zm_applied"))
		{
			message_vision ("$N皱了皱眉道：此次比武我未能静心尽力，希望重新来过。\n", me);
			return 1;
		}

                if( ob->query_temp("kunlun_zm_applied") != 1 )
			return 1;	
		...
		{
                        message_vision ("$N躬身对$n说道：恭请见过姜子牙。\n",me,ob);
...
void find_master (object me, object ob)
{
	...
	{
                message_vision ("$N见到姜子牙低声说了几句话，又抬起头朝$n使了个眼色。\n\n",me,ob);
...
*/
        return "昆仑山玉虚洞玉鼎极乐神仙"; //for kunlun only, others use above.
}
int rank_me()
{
...
if( me->query("title") == "昆仑山玉虚洞玉鼎极乐神仙")
//else if(me->query("title") == "昆仑山玉虚洞玉鼎极乐神姬")
...
if(me->query("gender") == "男性")
        ...
        {
                if( exp <= 20000 ){
                         me->set("title", "昆仑山扫雪小童");
                } else if(exp <= 50000 ){
                        me->set("title", "昆仑山护山道士");
                } else if(exp < 100000 ){
                        me->set("title", "昆仑山授功散仙");
                } else if(exp <= 200000 ){
                        me->set("title", "昆仑山执法掌教");
                } else if(exp <= 500000 ){
                        me->set("title", "昆仑山神机真人");
                } else if(exp <= 1000000 ){
                        me->set("title", "昆仑山虎鹤天师");
                } else if(exp <= 2000000 ){
                        me->set("title", "昆仑山南极仙翁");
                } else {
                        me->set("title", "昆仑山无极天尊");
                }
...
HIR""+ob->query("name")+"对"+me->query("name")+"说：你可以做"+me->query("title")+"。\n"NOR, ({me, me}));
        ...
        if( me->query("family/family_name") == "昆仑山玉虚洞")
        {
                if( exp <= 20000 ){
                        ...
                        me->set("title", "昆仑山护山道姑");
                } else if(exp < 100000 ){
                        me->set("title", "昆仑山内院主管");
                } else if(exp <= 200000 ){
                        me->set("title", "昆仑山授业仙姑");
                } else if(exp <= 500000 ){
                         me->set("title", "昆仑山神机圣姑");
                } else if(exp <= 1000000 ){
                        me->set("title", "昆仑山北极仙姑");
                } else if(exp <= 2000000 ){
                        me->set("title", "昆仑山玫瑰圣母");
                } else {
                        me->set("title", "昆仑山永宁神姬");
                }
